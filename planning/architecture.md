...
**Counter-affecting writes (binding, T-023):** a materialized counter reads an issue's project, its estimate, its live-or-trashed flag, and the **category of its state**. A write that changes `states.category`, or that reassigns `issues.state_id`, therefore changes what existing issues contribute without touching the issue rows the delta path watches. Because reads never recompute, nothing self-heals. Every such write owes either a pass through the issue-write choke-point (`runIssueWrite`, which drives the delta consumer) or an explicit `repairProjectProgress` for each affected project, and it owes it **inside the same transaction** as the write itself, so a failed repair cannot leave the schema edited and the counters stale. `tests/api/projects-choke-point.test.ts` asserts the source-tree inventory of raw `issues` writes, currently zero.

**The reconciliation sweep, and who calls it (T-025):** `repairProjectProgress` self-heals one project and runs from the delta consumer whenever a cache is legacy or absent. `repairAllProjects` is the workspace-wide sweep. It has no product caller and that is deliberate rather than an oversight: an operator facing reconcile belongs to M10 admin (T-019), and inventing an endpoint for it earlier would put a full table scan behind a route with no permission story. Until then it is exercised by `tests/api/projects-progress-seed.test.ts`, which perturbs every seeded project and requires the sweep to restore exactly what the seed derived. An unused backstop nothing can invoke is not a backstop, so if T-019 ships without a reconcile, delete the function rather than leave it as decoration.

**The demo bench derives its own caches (T-025):** `scripts/seed.ts` computes `progress_cache`, `progress_points_cache` and the closed cycle's `stats_snapshot` from the rows it inserts. It used to hand-write them, in the legacy two-field shape and with numbers that disagreed with those rows, and because reads never recompute the bench served them as fact. The seed uses raw SQL rather than the service, since node runs that file directly and cannot resolve the `@/` alias, so the test above pins the two implementations to each other.
